...
The rule governs what is written, not a campaign over what already exists: a position address already sitting in a perishable artifact — a task spec, a plan, a work-order — is pruned with that artifact, and sweeping for it buys nothing. What it forbids is a new one entering a surface that outlives its own numbering.
