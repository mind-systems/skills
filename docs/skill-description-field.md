...
Coherence requires every thing to be named by the artifact's own word. A divergence is live now: `roadmap-outline`'s skill-description calls its product "major milestones / high-level goals" and never "phase"; `roadmap-decompose` calls its own "milestones" too. The artifact grammar (both bodies, `roadmap-engine`, [skill-cycle](philosophy/skill-cycle.md)) is `### Phase N` (outline's product) and `N.M — task` (decompose's). The skill-description-field names two different tiers by one obsolete word and fails to name them by the words the artifact actually uses. "Milestone" is obsolete; the family has phase (headers) and task (`N.M`). Aligning the vocabulary — saying "phase" where outline makes phases, "task" where decompose makes tasks, retiring "milestone" — is a vocabulary edit, not a fact or a topology edit: the first application of the doctrine, not a rewrite of the skill-descriptions.
