...
2. **Write `.claude/commands/command-handoff.md`** with frontmatter:
   ...
   - `allowed-tools: Write Bash(ls *)` — exactly what the note path runs: `ls .ai-factory/notes/` to scan existing `<NN>` notes, `Write` to author the file (Write auto-creates the parent dir). **Declare nothing else** — no `Read`, no `Glob`, no `Bash(mkdir *)`; over-declaring just adds unused-tool noise. The default chat path uses no tools.

3. **Default behavior (no args):** emit the handoff prompt to **chat only** — the ~95% copy-paste case. No file written.

4. **Note trigger:** if `$ARGUMENTS` contains any of `note`, `ноут`, `давай ноут`, `с ноутом` → ALSO persist the handoff to disk. **The command writes the file itself (Write); it must not route this through `aif-note`** — aif-note reshapes content into its own note template and would lose the skeleton. Persist the exact handoff emitted to chat, in the step-6 skeleton format. Filing convention (same numbering rule as aif-note, applied inline): `<NN>` = next zero-padded two-digit prefix found by scanning `.ai-factory/notes/` for `[0-9][0-9]-*.md` and incrementing the highest; **if the directory is absent or empty, start at `01`** (no `mkdir` — `Write` auto-creates the parent dir); **`<slug>` derived semantically from the session's subject matter** (lowercase, hyphens), not a hardcoded `handoff`; English. Path: `.ai-factory/notes/<NN>-<slug>.md`. The default (no-arg) path stays chat-only and writes nothing.
