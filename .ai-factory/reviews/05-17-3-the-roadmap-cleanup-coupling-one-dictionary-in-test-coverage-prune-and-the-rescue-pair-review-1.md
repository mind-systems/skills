...
- Affects: 17.4 (`.ai-factory/specs/65-standalone-skills-conformance.md`) — residual `milestone` hits remain elsewhere in `src/skills/`: `detangle` (1), `roadmap-outline` (1, 17.2's deliberate user-input exemption), `aif-docs` (2). All owned by landed or planned tasks; nothing unowned. Noted so 17.4 reconciles against the current count rather than the spec's historical one.
