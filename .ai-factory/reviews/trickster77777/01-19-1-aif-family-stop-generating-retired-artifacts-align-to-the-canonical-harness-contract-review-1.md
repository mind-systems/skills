...
- Affects: `src/skills/aif/SKILL.md:5` — `allowed-tools` still grants no `Bash(ln *)` while `## AGENTS.md Generation` runs `ln -sfn CLAUDE.md AGENTS.md`. Already carried by contract line 19.2 with a matching guard about the shared L5 edit; correctly untouched here. [fixed]
