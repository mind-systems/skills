...
- Affects: the milestone's governing spec (`specs/38`) / the skill's own flow — Both the spec and Task 1 describe a retained `[x]` line as named "at invocation or at the confirmation step," but `roadmap-prune/SKILL.md` has no discrete confirmation step (its flow runs Step 0→8 then a commit-on-request block; it does not load `roadmap-engine`'s draft→confirm flow). In practice the only reliable retention channel is "at invocation," with the pre-commit working-tree review as an informal second chance. This wording is inherited verbatim from the ratified spec, so it is not a plan defect; making the "confirmation step" real would need a new step added — outside this milestone's file boundary and scope.
