...
- Affects: `.ai-factory/specs/05-orchestrator-artifacts-engine.md` (and the roadmap line 109) — The provenance clause "moved from spec `03`, substance verbatim — do not paraphrase differently from spec 03's wording" (Task 1 item 6, transcribed faithfully from spec 05 item 6 / its "What NOT to do") points at a source that no longer holds the text: spec `03-prune-harvest-deferred-observations.md` was already reduced to a *reference* ("the marker grammar … live in the orchestrator-artifacts engine (spec 05)", line 10). The authoritative grammar wording now lives only in spec 05 item 6 (and spec 04). This does not impede implementation — the plan inlines the full grammar verbatim and instructs "Follow spec 05 Edit 1 exactly," so the produced engine text is unambiguous — but the dangling "match spec 03's wording" pointer should be retargeted to spec 05 as the grammar's home. The plan faithfully implements a ratified spec; the imprecision originates in spec 05, which this milestone has no authority to rewrite. [dismissed]
