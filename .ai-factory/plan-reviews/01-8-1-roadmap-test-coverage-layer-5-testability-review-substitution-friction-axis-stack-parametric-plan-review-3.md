...
- Affects: roadmap task 17.3 — `SKILL.md` carries 3 pre-existing `milestone` occurrences (`:35`, `:50`, `:51`) outside the Layer 5 fence. This plan correctly constrains only *new* prose to registry vocabulary and leaves the sweep to 17.3, which the contract line schedules strictly after Phase 8 to avoid the same-file collision. Noted only so a reviewer seeing `milestone` in the post-change file does not read it as introduced by this task.
