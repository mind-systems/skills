...
*This handoff is the design source for the roadmap task **"agents: Editor"**
(`.ai-factory/specs/17-agents-editor-skill.md`) in the skills repo: the orchestrator
authors `src/agents/editor.md` from it — an **agent definition** (persistent subagent),
a sibling member of the `src/agents/` category task 16 establishes. The **architect** is
the other half of the pair, described in its own handoff,
`.ai-factory/handoffs/09-architect-paired-loop.md`.*
