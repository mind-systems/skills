...
## Rider — canonical RULES.md path convergence

This task is closed; the rider below extends it rather than amends its original Change/Verification, which stay as their own stratum. The orchestrator reads a hardcoded `.ai-factory/RULES.md` in every prompt and never reads a `rules/base.md` path — `tradeoxy_core`, `tradeoxy_gui`, and `tradeoxy_broker` already sit at the canonical path from this task's original pass; `tradeoxy_analyst`'s already-filtered `.ai-factory/rules/base.md` was the family's one outlier. It converges to `.ai-factory/RULES.md` in two steps: a `git mv` to the canonical path, then a de-duplication pass against `tradeoxy_analyst`'s own `CLAUDE.md` (and the `ARCHITECTURE.md` it points to) — one home per fact, so facts the frozen `## Stack` / `## Hard rules` / architecture doc already state (ports-and-adapters, pydantic wire validation, Decimal-as-string, pyright-strict, structured logging, the pytest/fake-ports strategy) drop from `RULES.md`, keeping only what has no other home: the "never swallow errors silently" rule and the `decide()` control-flow bullet (its cross-repo pointer to the broker's `PLRWorker`/`ExchangeIntentLifecycle` house pattern is unique to RULES even though the mechanism itself is now also in `ARCHITECTURE.md`). The now-empty `rules/` directory is removed. `tradeoxy_analyst`'s vestigial `.ai-factory/config.yaml` is removed outright — the orchestrator never reads it, and it is an artifact of aif skills outside our active set.
