...
**Invariant:** after the change, this task's own target (`roadmap-decompose-skeleton/SKILL.md`) drops out of the sweep entirely — the new Lens 1 contains neither "genuinely makes the surface testable" nor a `loads:` line ending in `test-philosophy` (it now ends in `polymorphism-philosophy`), confirmed by testing the exact replacement text against the sweep pattern directly. What the sweep still returns is this task's own spec (which quotes the old wording permanently as the problem it describes, exactly as `## Current state` does above) and this phase's own note (`151-…md`, which documents the pre-task state and is not invalidated by the file now saying something else — a phase note describes a moment, not a standing claim about the present). No other file in the repository quotes either passage, before or after this task.
