...
**Invariant:** after the change, `src/commands/command-pin-gaps.md`'s Blast-radius holes clause and `.ai-factory/specs/trickster77777/138-four-witnesses-each-trusted-where-it-is-blind.md` — phase 44's own note, not yet decomposed — both read the new `Repair:` wording, the rule, the sweep, and the invariant, never a sentence ordering an enumeration, in place of the old. What the sweep still returns beyond these two falls under the rule's own exceptions above — a spec's own current-state section recording the old wording as history, or a phase note or handoff recording what was true when it was written. The sweep locates candidates; reading decides.
