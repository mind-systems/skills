...
**Invariant:** the sweep returns this task's own target (`agent-architect/SKILL.md`), this task's own spec, this phase's own note `166-…`, the roadmap's own contract line for this task, and `.ai-factory/notes/01-architect-buffer.md` — a different architect's own buffer, out of this task's scope and untouched, per the standing rule that no architect reads or edits another's buffer. No other file, before or after this task.

The target itself carries the phrase in more than one place, each checked: the passage being replaced, and a separate occurrence in the "Leaving both buffers live" paragraph earlier in the same section — "losing a working editor is already a named cost ('Losing the editor is never fatal; losing it silently is the defect')" — which quotes this passage's own closing sentence as support for preferring re-pointing over discarding a live-but-wrong-pointed editor. That sentence is untouched by this task's replacement (it is the one clause carried forward verbatim, confirmed against `## What must be true after` above), so the quote stays accurate and that paragraph's own argument is unaffected. This corrects an earlier version of this section, which searched for the un-wrapped phrases directly, found only the target's own passage as a result of the search silently missing the wrapped second occurrence, and did not check whether that occurrence's argument survives the change.
