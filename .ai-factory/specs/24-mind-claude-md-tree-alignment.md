...
This task is closed; the rider below extends it rather than amends its original Change/Verification, which stay as their own stratum. A later sweep closes two gaps this task's original pass left open. First, the orchestrator reads a hardcoded `.ai-factory/RULES.md` in every prompt and never reads `.ai-factory/config.yaml` at all — the vestigial `.ai-factory/config.yaml` files in `mind_web` and `camera_ppg_kit`, the only two mind subrepos that carried one, are removed outright: they are aif-skill artifacts the orchestrator never reads, belonging to aif skills outside our active set. Second, `mind_web` and `camera_ppg_kit` were this family's last two subrepos still on the orchestrator-invisible `.ai-factory/rules/base.md` path — their rules are filtered by the counter-default criterion and the survivors converted to the canonical `.ai-factory/RULES.md`, and the now-empty `rules/` directories are removed. The converted `RULES.md` files are also de-duplicated against their own `CLAUDE.md` — one home per fact — so only RULES-unique content survives; where `CLAUDE.md` already states a fact, `RULES.md` drops it rather than restating it.
