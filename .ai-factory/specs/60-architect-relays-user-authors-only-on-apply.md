...
- **Relay is scoped to editor-bound work, never every message.** Deciding which user messages are work for the editor (a target to analyze, or an approved change to apply) versus conversation directed at the architect itself (a fork answer, "go", a commit order, a correction, a question about approach) is the architect's own standing judgment — it works today and the diet must **preserve** it, not flatten it into "forward everything." Only editor-bound work messages are relayed; architect-directed conversation the architect acts on itself. State this as a one-line scope, not a taxonomy of message types — do not re-specify the judgment, only name that it holds.
- **An editor's scope question goes to the user, never to the architect's inference.** When the editor flags a clarification back — "which skeleton pass?", "what is the scope of phase 8?" — that is correct editor behavior; the architect carries the question to the user and relays the user's answer, and must never resolve the ambiguity with its own guess. Seeding via an "answer" is the same contamination the relay model prevents, re-entering through the back door.
- **Terse relay leans on the persistent editor; re-brief a cold one.** The relay model depends on spawn-once harder than the old apply-only model did — a bare "phase 14" resolves only because the editor carries the session. A skill-by-reference relay carries its own path read and a full user message is self-contained, but before a *terse* relay to a fresh or respawned editor the architect re-briefs it from the digest the spawn-once discipline already records.
...
- `editor.md` is dieted in its own task 7.2 (shared apply/analysis vocabulary) — not bundled here; the two rewrites land as a coherent pair, 7.1 then 7.2.
