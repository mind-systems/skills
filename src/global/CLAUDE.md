...
When the project has `.ai-factory/ROADMAP.md`, it is the entry map of **time**: the seam between `[x]` and `[ ]` is where the project lives now — aim there. A `[x]` line is always history, never current state: it describes the moment of its own planning, a later line on the same surface supersedes an earlier one, and the present is verified only against the files. Its counterpart `.ai-factory/ARCHITECTURE.md` is the entry map of **space** — module boundaries, the chosen pattern, and the compacted history in `## Features`; the two maps together orient a cold session before any skill is invoked. Named roadmaps under `.ai-factory/roadmaps/` branch that time map — per-developer working buffers, each opening with an `> Owner:` first line — and a multiuser project's present is the union of their `[x]`/`[ ]` seams, so entry into it starts by enumerating that directory.
